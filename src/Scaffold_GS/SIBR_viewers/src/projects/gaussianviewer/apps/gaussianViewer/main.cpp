...
#define PROGRAM_NAME "Scaffold-GS Viewer"
...
int main(int ac, char** av) 
{
	...
	rng = findArg(cfgLine, "add_opacity_dist");
	bool add_opacity_dist = true;
	if (rng.first > 0)
	{
		add_opacity_dist = cfgLine.substr(rng.first, rng.second - rng.first).find("True") != -1;
	}

	rng = findArg(cfgLine, "add_cov_dist");
	bool add_cov_dist = true;
	if (rng.first > 0)
	{
		add_cov_dist = cfgLine.substr(rng.first, rng.second - rng.first).find("True") != -1;
	}

	rng = findArg(cfgLine, "add_color_dist");
	bool add_color_dist = true;
	if (rng.first > 0)
	{
		add_color_dist = cfgLine.substr(rng.first, rng.second - rng.first).find("True") != -1;
	}
