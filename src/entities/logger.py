...
class Logger(object):
    ...
    def vis_mapping_iteration(self, frame_id, iter, color, depth, gt_color, gt_depth, seeding_mask=None, interval=1) -> None:
        """
        ...
        # 获取深度图的尺寸（高度、宽度）
        depth_height, depth_width = depth_np.shape[:2]
        dpi = 300  # 设置所需分辨率

        # 创建无边框画布，并设置正确尺寸
        fig = plt.figure(frameon=False)
        fig.set_size_inches(depth_width / dpi, depth_height / dpi)
        ...
        # 显示深度图（禁用插值以保证像素精度）
        ax.imshow(depth_np, cmap="plasma", vmin=0, vmax=100, interpolation='nearest')

        # 保存图像（禁用边框裁剪，确保像素尺寸精确）
        plt.savefig(
            str(self.output_path / "mapping_depth" / f'{frame_id:04d}.png'),  # 建议PNG格式避免压缩损失
            dpi=dpi,
            bbox_inches='tight',
            pad_inches=0,
            facecolor="auto"
        ...
        cv2.imwrite(str(self.output_path / "mapping_rgb" / f'{frame_id:04d}.png'), color_np[:,:,[2,1,0]]*255)
        ...
        axs[0, 0].imshow(gt_depth_np, cmap="jet", vmin=0, vmax=100)
        axs[0, 0].set_title('Input Depth', fontsize=16)
        axs[0, 0].set_xticks([])
        axs[0, 0].set_yticks([])
        axs[0, 1].imshow(depth_np, cmap="plasma", vmin=0, vmax=100)
