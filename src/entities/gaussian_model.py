...
from src.Scaffold_GS.utils.general_utils import inverse_sigmoid, get_expon_lr_func
from torch import nn
import os
from src.Scaffold_GS.utils.system_utils import mkdir_p
from plyfile import PlyData, PlyElement
from simple_knn._C import distCUDA2
from src.Scaffold_GS.utils.graphics_utils import BasicPointCloud
from src.Scaffold_GS.utils.general_utils import strip_symmetric, build_scaling_rotation
from src.Scaffold_GS.scene.embedding import Embedding
import open3d as o3d

from src.gsr.se3.numpy_se3 import from_xyzquat
...
class GaussianModel:
    ...
    def __init__(self,sh_degree:int =3, isotropic=False,
        ...
        # self._xyz = torch.empty(0).cuda()  # self._anchor self._offset
        # self._features_dc = torch.empty(0).cuda() # self._anchor_feat
        # self._features_rest = torch.empty(0).cuda() # self._anchor_feat
        # self.xyz_gradient_accum = torch.empty(0)  #  self.offset_gradient_accum
        # self.denom = torch.empty(0)   #    self.anchor_demon
    ...
    def capture_dict(self):
        return {
            ...
            "anchor": self._anchor.clone().detach().cpu(),
            "offset": self._offset.clone().detach().cpu(),
            "anchor_feat": self._anchor_feat.clone().detach().cpu(),
            ...
            "offset_gradient_accum": self.offset_gradient_accum.clone().detach().cpu(),
            "offset_denom": self.offset_denom.clone().detach().cpu(),
    ...
    def densification_postfix(self, new_anchor, new_offset, new_anchor_feat,
        ...
        self.offset_gradient_accum = torch.zeros((self.get_xyz().shape[0], 1), device="cuda")
        self.offset_denom = torch.zeros((self.get_xyz().shape[0], 1), device="cuda")
        self.max_radii2D = torch.zeros(
            (self.get_xyz().shape[0]), device="cuda")

    def add_densification_stats(self, viewspace_point_tensor, update_filter):
        self.offset_gradient_accum[update_filter] += torch.norm(
    ...
    def add_points(self, pcd: o3d.geometry.PointCloud, global_scale_init=True):
        ...
        new_rotation = nn.Parameter(rots.requires_grad_(False))
        new_opacity = nn.Parameter(opacities.requires_grad_(False))
        self.densification_postfix(
            new_anchor,
            new_offset,
            new_anchor_feat,
    ...
    def fix_mlp(self, training_args, exposure_ab=None):
    ...
    def training_setup_camera(self, cam_rot, cam_trans, cfg, exposure_ab=None):
        ...
        if self.use_feat_bank:
            l = [
                {'params': [self._anchor], 'lr': 0.0,
                 "name": "anchor"},
                {'params': [self._offset], 'lr': 0.0,
                 "name": "offset"},
                ...
                {'params': self.mlp_opacity.parameters(), 'lr': 0.0,
                 "name": "mlp_opacity"},
                {'params': self.mlp_feature_bank.parameters(), 'lr': 0.0,
                 "name": "mlp_featurebank"},
                {'params': self.mlp_cov.parameters(), 'lr': 0.0, "name": "mlp_cov"},
                {'params': self.mlp_color.parameters(), 'lr': 0.0, "name": "mlp_color"},
                {'params': self.embedding_appearance.parameters(), 'lr': 0.0,
                 "name": "embedding_appearance"},
            ]
        elif self.appearance_dim > 0:
            l = [
        ...
        l.extend([            {"params": [cam_rot], "lr": cfg["cam_rot_lr"],
                "name": "cam_unnorm_rot"},
