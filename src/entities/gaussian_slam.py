...
# from src.Scaffold_GS.scene import GaussianModel
...
class GaussianSLAM(object):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.reg.set_max_correspondence_distance(1.0)
        ...
        self.loss_path = os.path.join( self.output_path, 'psnr.csv')
    ...
    def _setup_output_path(self, config: dict) -> None:
        ...
        os.makedirs(self.output_path / "mapping_rgb", exist_ok=True)
        os.makedirs(self.output_path / "mapping_depth", exist_ok=True)
        ...
        if self.submap_using_motion_heuristic:
            if exceeds_motion_thresholds(
                self.estimated_c2ws[frame_id], self.estimated_c2ws[self.new_submap_frame_ids[-1]],
                    rot_thre=50, trans_thre=5):  # rot_thre=50, trans_thre=0.5
    ...
    def start_new_submap(self, frame_id: int, gaussian_model_old: GaussianModel) -> None:
        ...
        gaussian_model.mlp_color = gaussian_model_old.mlp_color
        gaussian_model.mlp_cov = gaussian_model_old.mlp_cov
        gaussian_model.mlp_opacity = gaussian_model_old.mlp_opacity
    ...
    def run(self) -> None:
        ...
        for frame_id in range(len(self.dataset)):
            ...
            #     self.reg.set_input_target(pygicp.downsample(self.dataset[frame_id][-2], 0.25))
            #     delta = np.identity(4)
            # else:
            #     estimated_c2w, delta, exposure_ab = self.tracker.track_icp(self.reg,
            #         frame_id, gaussian_model,
            #         torch2np(self.estimated_c2ws[torch.tensor([0, frame_id - 2, frame_id - 1])]))
            # gt pose
            estimated_c2w = self.dataset[frame_id][-3]
            print("pose", estimated_c2w)
            exposure_ab = torch.nn.Parameter(torch.tensor(
                0.0, device="cuda")), torch.nn.Parameter(torch.tensor(0.0, device="cuda"))
            # self.reg.set_input_target(pygicp.downsample(self.dataset[frame_id][-2], 0.25))
            delta = np.identity(4)
            ...
            gt_poses.append(self.dataset[frame_id][3])

            # traj = np.array([x[:3, 3] for x in poses])
            # pred = np.array([x[:3, 3] for x in self.estimated_c2ws[0:frame_id+1]])
            # gt = np.array([x[:3, 3] for x in gt_poses[0:frame_id+1]])
            ...
            if frame_id in self.mapping_frame_ids:
                ...
                gaussian_model.fix_mlp(self.opt, exposure_ab)
                ...
                with open(self.loss_path, 'a', newline='') as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow([frame_id, opt_dict['psnr_render'], opt_dict['ssim_render']])
