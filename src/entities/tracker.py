...
class Tracker(object):
    ...
    def track(self, frame_id: int, gaussian_model: GaussianModel, prev_c2ws: np.ndarray) -> np.ndarray:
        ...
        """
        _, image, depth, gt_c2w , points, _ = self.dataset[frame_id]
    ...
    def track_icp(self, gicp, frame_id: int, gaussian_model: GaussianModel, prev_c2ws: np.ndarray) -> np.ndarray:
        ...
        points = pygicp.downsample(points, 0.2)
        # Set initial pointcloud to target points
        gicp.set_input_source(points)
        delta = gicp.align()
        gicp.swap_source_and_target()

        # final_c2w = last_c2w.dot(delta)
        final_c2w = last_c2w @ delta
        ...
        # final_c2w = torch.inverse(torch.from_numpy(reference_w2c) @ best_w2c)
        # final_c2w[-1, :] = torch.tensor([0., 0., 0., 1.], dtype=final_c2w.dtype, device=final_c2w.device)
        return (final_c2w), delta, exposure_ab