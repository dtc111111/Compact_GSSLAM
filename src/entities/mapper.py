...
# from src.Scaffold_GS.gaussian_renderer import prefilter_voxel, render, network_gui
...
class Mapper(object):
    ...
    def optimize_submap(self, keyframes: list, gaussian_model: GaussianModel, iterations: int = 100) -> dict:
        ...
        while iteration < iterations + 1:
            ...
            # print("col_loss",color_loss,"de_loss",depth_loss)
            reg_loss = isotropic_loss(gaussian_model.get_scaling())
            total_loss = color_loss + 0.05*depth_loss + reg_loss
            total_loss.backward()
    ...
    def grow_submap(self, gt_depth: np.ndarray, estimate_c2w: np.ndarray, gaussian_model: GaussianModel,
        ...
        gaussian_points = gaussian_points.to(reused_pts_ids.device)
    ...
    def map(self, frame_id: int, estimate_c2w: np.ndarray, gaussian_model: GaussianModel, is_new_submap: bool, exposure_ab=None) -> dict:
        ...
        _, gt_color, gt_depth, _, _, dense_depth = self.dataset[frame_id]
        # _, gt_color, gt_depth, _ = self.dataset[frame_id]
        # dense_depth = gt_depth.copy()
        estimate_w2c = np.linalg.inv(estimate_c2w)
        print("0", gaussian_model.get_size())
        ...
        mask = np.zeros((self.dataset.height, self.dataset.width), dtype=bool)
        # 固定步长采样
        mask[::2, ::2] = True
        seeding_mask = mask ##| seeding_mask
        pts = self.seed_new_gaussians(
            gt_color, dense_depth, self.dataset.intrinsics, estimate_c2w, seeding_mask, is_new_submap)

        filter_cloud = isinstance(self.dataset, (TUM_RGBD, ScanNet)) and not is_new_submap

        # if gaussian_model.get_size() == 0:
        # gaussian_model._anchor_feat = gaussian_model._anchor_feat*0.1
        # if frame_id == 0:
        new_pts_num = self.grow_submap(dense_depth, estimate_c2w, gaussian_model, pts, filter_cloud)
        # else:
        #     new_pts_num  = 0
        ...
        start_time = time.time()
        print("1", gaussian_model.get_size())
        ...
        with torch.no_grad():
            ...
            ssim_value = ssim(image_vis, keyframe["color"]).item()
            opt_dict["psnr_render"] = psnr_value
            opt_dict["ssim_render"] = ssim_value
