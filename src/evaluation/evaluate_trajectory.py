...
def evaluate_trajectory(estimated_poses: np.ndarray, gt_poses: np.ndarray, output_path: Path):
    ...
    ax = plot_2d(
        estimated_t[:,[0,2,1]], label=f"ate-rmse: {round(ate_rmse * 100, 2)} cm", color="orange")
    ax = plot_2d(estimated_t_aligned[:,[0,2,1]], ax,
                 label=f"ate-rsme (aligned): {round(ate_rmse_aligned * 100, 2)} cm", color="lightskyblue")
    ax = plot_2d(gt_t[:,[0,2,1]], ax, label="GT", color="green")
    ax.legend()
