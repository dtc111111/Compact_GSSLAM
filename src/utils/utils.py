...
def render_gaussian_model_(gaussian_model, render_settings,
...
def generate_neural_gaussians(viewpoint_camera, pc: GaussianModel, visible_mask=None, is_training=False):
    ...
    ob_view = anchor - viewpoint_camera.campos
    # dist
    ...
    if pc.appearance_dim > 0:
        ...
        if pc.add_color_dist:
            ...
            # color = pc.get_color_mlp(pc.get_features())
            # print(cat_local_view_wodist.shape)
            # print(color.shape)
...
#            retain_grad=False):
def render_gaussian_model(pc, render_settings,
                          override_means_3d=None, override_means_2d=None,
                          override_scales=None, override_rotations=None,
                          override_opacities=None, override_colors=None,retain_grad=False,visible_mask=None):
    """
    ...
    """
    # print(render_settings.campos)
    is_training = pc.get_color_mlp.training

    if is_training:
        xyz, color, opacity, scaling, rot, neural_opacity, mask = generate_neural_gaussians(render_settings, pc,
                                                                                            visible_mask,
    ...
    #             "radii": radii,
    #             }
    # print(color.min(), color.max(),depth.min(),depth.max(),alpha.min(),alpha.max())
    return {"color": color, "depth": depth, "radii": radii, "means2D": screenspace_points, "alpha": alpha}
