...
def transformed_params2rendervar(rvq_model, params, transformed_pts, curr_time_idx, rvq_iter=False, eval_or_tracking = False):
    l_vqsca=0
    l_vqrot=0
    get_scaling = torch.exp(torch.tile(params['log_scales'], (1, 3)))
    get_rotation = F.normalize(params['unnorm_rotations'])
    get_opacity = torch.sigmoid(params['logit_opacities'])

    if not eval_or_tracking and rvq_iter:  # Apply R-VQ
        scales, _, l_vqsca = rvq_model.vq_scale(get_scaling.unsqueeze(0))
        rotations, _, l_vqrot = rvq_model.vq_rot(get_rotation.unsqueeze(0))
    ...
    if not eval_or_tracking and (curr_time_idx+1) % 5 == 0:  # Apply mask
    ...
    return rendervar, l_vqsca + l_vqrot
