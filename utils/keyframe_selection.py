...
def get_pointcloud(depth, intrinsics, w2c, sampled_indices):
    ...
    xx = (sampled_indices[:, 1] - CX)/FX  # x/z in cam axis, ([1600])
    yy = (sampled_indices[:, 0] - CY)/FY  # y/z in cam axis, ([1600])
    depth_z = depth[0, sampled_indices[:, 0], sampled_indices[:, 1]]  # ([1600])

    # Initialize point cloud
    pts_cam = torch.stack((xx * depth_z, yy * depth_z, depth_z), dim=-1)  # positions in cam axis, (x,y,z), shape = (1600, 3)
    pts4 = torch.cat([pts_cam, torch.ones_like(pts_cam[:, :1])], dim=1)
    c2w = torch.inverse(w2c)
    pts = (c2w @ pts4.T).T[:, :3]  # world axis

    # Remove points at camera origin
    A = torch.abs(torch.round(pts, decimals=4))  # shape = (1600, 3)
    B = torch.zeros((1, 3)).cuda().float()  # shape = (1, 3)
