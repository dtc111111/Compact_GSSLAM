...
def report_loss(losses, wandb_run, wandb_step, tracking=False, mapping=False):
    ...
    loss_dict = {'Loss': losses['loss'].item(),
                 ...
                 'Mask Loss': losses['mask'].item(),
                 'R-VQ Loss': losses['vqloss']
                 }
    if tracking:
        tracking_loss_dict = {}
...
def report_progress(hashgrid_model, params, data, i, progress_bar, iter_time_idx, sil_thres, every_i=1, qual_every_i=1, 
    ...
    if i % every_i == 0 or i == 1:
        ...
        rendervar, vqloss = transformed_params2rendervar(hashgrid_model, params, transformed_pts,i, rvq_iter=False, eval_or_tracking= True)
...
def eval_online(dataset, all_params, num_frames, eval_online_dir, sil_thres,
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        # Initialize Render Variables
        rendervar, vqloss = transformed_params2rendervar(hashgrid_model, params, transformed_pts, time_idx, rvq_iter=False, eval_or_tracking= True)
...
def eval(hashgrid_model, dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        # Initialize Render Variables
        rendervar, vqloss = transformed_params2rendervar(hashgrid_model, final_params, transformed_pts, time_idx, rvq_iter=False, eval_or_tracking= True)
    ...
    print("Average Time:", total_time/total, "s/frame")
    try:
