...
def in_frustum(params, cam, time_idx):
    transformed_pts = transform_to_frame(params, time_idx, gaussians_grad=False, camera_grad=False)
    viewmatrix = cam.viewmatrix.transpose(1, 2).squeeze(0)
    projmatrix = cam.projmatrix.transpose(1, 2).squeeze(0)
    pts_ones = torch.ones(transformed_pts.shape[0], 1).cuda().float()
    pts4 = torch.cat((transformed_pts, pts_ones), dim=1)

    p_hom = (projmatrix @ pts4.T).T
    hom_w = p_hom[:,3]
    p_w = 1 / hom_w
    p_w = p_w.view(-1, 1)
    p_proj = p_hom * p_w
    p_view = (viewmatrix @ pts4.T).T
    view_z = p_view[:,2]

    proj_x = p_proj[:,0]
    proj_y = p_proj[:,1]
    x_in = torch.logical_and(proj_x >= -0.9, proj_x <= 0.9)
    y_in = torch.logical_and(proj_y >= -0.9, proj_y <= 0.9)
    is_in = torch.logical_and(x_in, y_in)
    return torch.logical_and((view_z > 0.2),is_in)
    # z_in =  torch.logical_and((view_z > 0.2), (view_z < 1.0))
    # return torch.logical_and(z_in,is_in)


def prune_gaussians(params, variables, optimizer, iter, prune_dict, time_idx, cam, iter_time_idx):
    ...
    if iter == prune_dict['start_after'] and ((time_idx+1) % prune_dict['mask_per'] == 0 and time_idx >= prune_dict['mask_start'] and time_idx <= prune_dict['mask_stop'] ) :
        to_mask = torch.logical_and((torch.sigmoid(params['mask']) <= 0.01).squeeze(), in_frustum(params, cam, time_idx))
        # to_mask = torch.logical_and((torch.sigmoid(params['mask']) <= 0.01).squeeze(), is_new(params, new_pt_num))
        params, variables = remove_points(to_mask, params, variables, optimizer)
        torch.cuda.empty_cache()
    if iter ==  prune_dict['reset_iter'] and ((time_idx+1) % prune_dict['mask_reset_cycle'] == 0) :    
        params['mask'] = torch.nn.Parameter(torch.ones((params['mask'].shape[0], 1), device="cuda").cuda().float().contiguous().requires_grad_(True))
    return params, variables
