	...
	bool prefiltered)
{
	...
	float4 p_hom = transformPoint4x4(p_orig, projmatrix);  // p_hom = projmatrix @ p_orig
	...
	float3 cov = computeCov2D(p_orig, focal_x, focal_y, tan_fovx, tan_fovy, cov3D, viewmatrix); // Sigma'
	...
	float3 conic = { cov.z * det_inv, -cov.y * det_inv, cov.x * det_inv };  // Invert covariance
	...
	float2 point_image = { ndc2Pix(p_proj.x, W), ndc2Pix(p_proj.y, H) };  // x' = ((x + 1) * W - 1) * 0.5
