...
int CudaRasterizer::Rasterizer::forward(
	std::function<char* (size_t)> geometryBuffer,  // geomFunc
	std::function<char* (size_t)> binningBuffer,  // binningFunc
	std::function<char* (size_t)> imageBuffer,  //imgFunc
	const int P, int D, int M,  // P, degree, M,
	const float* background,  
	const int width, int height,  // W, H,
	const float* means3D,  
	const float* shs,  // sh
	const float* colors_precomp,  // colors 
	const float* opacities,  
	const float* scales,  
	const float scale_modifier,  
	const float* rotations,  // rotations
	const float* cov3D_precomp,  //cov3D_precomp
	const float* viewmatrix,  
	const float* projmatrix,
	const float* cam_pos,  // campos
	...
	int* radii)
{
	const float focal_y = height / (2.0f * tan_fovy);  // tan_fovy = (h/2) / focal_y
